...
class WishListService{
    ...
    getwishlist=asyncHandler(async(req:Request,res:Response,next:NextFunction)=>{
        const user: UserInterface | null=await UserSchema.findById(req.user?.id).populate(`wishlist`)
            if(!user) {
    ...
    addTowishlist=asyncHandler(async(req:Request,res:Response,next:NextFunction)=>{
        const user: UserInterface | null=await UserSchema.findByIdAndUpdate(req.user?.id,{$addToSet:{wishlist:req.body.productId}},{new:true})
            if(!user) {
                return next(new ApiErrors(`${req.__('not_found')}`,404));
            }                
    ...
    removeFromwishlist=asyncHandler(async(req:Request,res:Response,next:NextFunction)=>{
        const user: UserInterface | null=await UserSchema.findByIdAndUpdate(req.user?.id,{$pull:{wishlist:req.params.productId}},{new:true})
            if(!user) {
                return next(new ApiErrors(`${req.__('not_found')}`,404));
            }
            await user.populate('wishlist')
